...
def offer_lunzheng_zhaocuoxicuo(section):
    conn = MySQLdb.Connect(host='127.0.0.1', user='root', passwd='123456', db='gyc_f_e', port=3306, charset='utf8')
    cur = conn.cursor()
    sql1='''select content from lunzheng_zhaocuoxicuo where section_name='%s' ''' % (section)
    cur.execute(sql1)
    all= cur.fetchall()
    all_content=all[0][0]
    ...
    sql2='''select section_name2,section_name3 from lunzheng_zhaocuoxicuo_name where section_name1='%s' ''' % (section)
    cur.execute(sql2)
...
def lunzheng_zhaocuoxicuo1():
    sys.exit(offer_lunzheng_zhaocuoxicuo(sys.argv[1]))

if __name__ == '__main__':
    #offer_lunzheng_zhaocuoxicuo("套路化析错")
    lunzheng_zhaocuoxicuo1()
