...
def logic_show_finish_zhongdian(user_name,type,wrong):
    conn = MySQLdb.Connect(host='127.0.0.1', user='root', passwd='123456', db='gyc_f_e', port=3306, charset='utf8')
    cur = conn.cursor()
    sql1 = 'select * from logic_storage_zhongdian_type'
    ...
    for x in all_data1:
        ...
        if str(x[1]).decode('utf-8')==str(user_name).decode('utf-8') and str(x[2]).decode('utf-8')== str(type).decode('utf-8'):
...
#logic_show_finish_zhongdian('lxq','logic_zonghe',1)


def logic_show_finish_zhongdian1():
    sys.exit(logic_show_finish_zhongdian(sys.argv[1],sys.argv[2],sys.argv[3]))

if __name__ == '__main__':
    # logic_show_finish_zhongdian('lxq','命题逻辑',1)
    logic_show_finish_zhongdian1()
