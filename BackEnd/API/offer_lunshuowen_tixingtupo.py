...
def offer_lunzheng_tixingtupo(section):
    conn = MySQLdb.Connect(host='127.0.0.1', user='root', passwd='123456', db='gyc_f_e', port=3306, charset='utf8')
    cur = conn.cursor()
    sql1='''select content from lunshuowen_tixingtupo where section_name='%s' ''' % (section)
    cur.execute(sql1)
    all= cur.fetchall()
    all_content=all[0][0]
    ...
    sql2='''select section_name2,section_name3 from lunshuowen_tixingtupo_name where section_name1='%s' ''' % (section)
    cur.execute(sql2)
...
def offer_lunzheng_tixingtupo1():
    sys.exit(offer_lunzheng_tixingtupo(sys.argv[1]))

if __name__ == '__main__':
    #offer_lunzheng_tixingtupo("命题作文")
    offer_lunzheng_tixingtupo1()
