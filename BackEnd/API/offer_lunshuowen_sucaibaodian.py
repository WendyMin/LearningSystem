...
def offer_lunshuowen_sucaibaodian(section):
    conn = MySQLdb.Connect(host='127.0.0.1', user='root', passwd='123456', db='gyc_f_e', port=3306, charset='utf8')
    cur = conn.cursor()
    sql1='''select content from lunshuowen_sucaibaodian where section_name='%s' ''' % (section)
    cur.execute(sql1)
    all= cur.fetchall()
    all_content=all[0][0]
    ...
    sql2='''select section_name2,section_name3 from lunshuowen_sucaibaodian_name where section_name1='%s' ''' % (section)
    cur.execute(sql2)
...
def offer_lunshuowen_sucaibaodian1():
    sys.exit(offer_lunshuowen_sucaibaodian(sys.argv[1]))

if __name__ == '__main__':
    #offer_lunshuowen_sucaibaodian("世界人物素材")
    offer_lunshuowen_sucaibaodian1()
